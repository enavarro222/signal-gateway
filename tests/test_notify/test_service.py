...
@pytest.mark.asyncio
async def test_async_send_message_no_message(mock_hass, mock_signal_client):
    ...
    await service.async_send_message(message=None, target="+1111111111")

    # Should not send when message is None
    mock_signal_client.send_message.assert_not_called()


@pytest.mark.asyncio
async def test_async_send_message_empty_message(mock_hass, mock_signal_client):
    """Test send with empty message."""
    from custom_components.signal_gateway.notify.service import (
        SignalGatewayNotificationService,
    )
    ...
    await service.async_send_message(message="", target="+1111111111")

    # Should not send when message is empty
    mock_signal_client.send_message.assert_not_called()
